...
export default function Login() {
  ...
  return (
    <div className="flex-1 text-sm sm:text-xs flex flex-col justify-center items-center gap-2 sm:gap-4">
      ...
      { error && <div className="w-full max-w-[40ch] text-center border-rose-400 border border-solid text-rose-400 py-2">{error}</div>}
      <input
        value={email} 
      ...
      <h2 className="duration-300 hover:scale-125 select-none cursor-pointer" onClick={()=>setIsLoggingIn(!isLoggingIn)}> {!isLoggingIn ? "Login" : "Register"}</h2>
    </div>
  );
}
